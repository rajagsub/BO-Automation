...
#        second_line = 'Emergency Room (Institutional)'
        for j in range(0, bpd_pdf_data_pages_len):
            if (bpd_ref[i] in bpd_pdf_data_pages[j] and (covered in bpd_pdf_data_pages[j]) and (second_line in bpd_pdf_data_pages[j] or second_line in bpd_pdf_data_pages[j+1])):
...
def write_df_to_csv(planname):
    ...
    dict = {'BO Ref':bo_ref, 'BPD Ref':bpd_ref, 'BPD Network':bpd_network, 'BPD Second':bpd_second_line, 'BO Pct':bo_pct_val, 'BPD Pct':bpd_pct_val, 'Output':bo_bpd_compare}
...
# Reading paths file where PDF file locations are stored    
    path_file = r"C:\Users\af40411\PycharmProjects\BO_PDF_Extraction\Final_BO_vs_BPD_Automation\PPO Plan\PPO_Path.txt"
    ...
    mapping_file = r"C:\Users\af40411\PycharmProjects\BO_PDF_Extraction\Final_BO_vs_BPD_Automation\PPO Plan\Mapping-1YAE.txt"
